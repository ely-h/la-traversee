...
public class PlayerCollision : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            isInfected = true;
            Debug.Log($"Joueur {playerId} est infect� !");

            GetComponent<SpriteRenderer>().color = new Color(0.31f, 0.41f, 0.13f);

            // Changer le tag du joueur pour qu'il soit trait� comme un ennemi
            ...
            // Dire au serv node.js que joueur est infect�
        ...
        // Gestion de l'arriv�e au bunker
        if (!isInfected && other.CompareTag("Bunker"))
        {
            ...
            // Env au serv node.js que le joueur est en s�curit�
            if (netManager != null && netManager.socket != null)
            {
